...
export const Accordion = ({data, first, last, updateProgress}: Props) => {
    ...
    return (
        <S.Wrapper>
            <S.ParentDiv first={first} last={last} open={shown} onClick={handleToggle}>
           ...
           {shown && (
            ...
            {data.tasks.map((task, i) =>(
                <S.TaskDiv key={i} last={last} open={shown} lastCheck={data.tasks.length - 1 == i}>
                    <S.Input id={task.description} 
                    type='checkbox' 
